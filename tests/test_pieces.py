...
class test_Pieces(unittest.TestCase):
    ...
    def test_get_possible_moves_pawn(self):
        ...
        pawn = Pawn(init_pos, White(),  self.boardsize)
    ...
    def test_get_possible_moves_king(self):
        ...
        """Test the possible moves."""
        init_pos = Position(0, 0, self.boardsize)
        king = King(init_pos, White(),  self.boardsize)
        pos_moves = Moves(Position(0, 1, self.boardsize), Position(1, 1, self.boardsize), Position(1, 0, self.boardsize))
        self.assertEqual(len(king.possible_moves()), len(pos_moves))
        
        init_pos = Position(4, 4, self.boardsize)
        pawn = King(init_pos, White(),  self.boardsize)
